...
We need to specify the name of the folder and the name of the csv file
...
Write them below:
What does the legal status mean? 
Are the addresses in San Francisco? 
What does site order mean? 

How many count of each observation by legal status? SHould we focus on the highest trees? 
```{r}
unique(sfo_trees_df$legal_status)
Graph_data<- sfo_trees_df |>
...
```{r}
# Barplot
ggplot(Graph_data, aes(x=legal_status, y=tree_count)) +
...
What does the `%in%` do? Write an example to explain the `%in%` operator to your neighbor using the empty code chunk below. It specifies what values in the caretaker column you wish to keep in the new data frame
